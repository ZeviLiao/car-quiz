...
### v1.1.0 (2025-01-09)
**🐛 重大Bug修復**
- **修復答對題目重複出現問題**：重構 `loadQuestions()` 函數執行順序，確保已答對題目正確排除
- **修復錯題選項無法顯示問題**：重寫 `selectAnswerFilter()` 邏輯，改用題目類型匹配替代錯誤的ID交集
- **修復題目重複計數問題**：避免同一題目同時出現在未答和錯題列表中造成數量異常
- **改善數據持久化**：調整載入順序，確保標記題目在題庫載入前正確初始化

**✨ 邏輯優化**
- 答對題目現在會立即從所有可用列表中排除，不再重複出現
- 錯題功能完全修復，答錯後可正確選擇「只測驗答錯的題目」選項
- 優化進度統計顯示，避免重複計數導致的數字異常
