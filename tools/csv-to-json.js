...
var CsvToJsonConverter = /** @class */ (function () {
    ...
    CsvToJsonConverter.prototype.getExplanationForQuestion = function (text, correctAnswer) {
        if (correctAnswer !== 'X') {
        ...
        // 智能匹配 - 清理對應表的key並比較
        for (var key in this.explanationMap) {
            var cleanKey = key
                .replace(/[，。、：；]/g, '') // 移除中文標點
                .replace(/\s+/g, '') // 移除所有空格
                .trim();
            ...
            if (cleanText.length > 10 && cleanKey.length > 10) {
                var similarity = this.calculateSimilarity(cleanText, cleanKey);
                if (similarity > 0.8) {
                    return this.explanationMap[key];
                }
            }
    ...
    // 計算字符串相似度
    CsvToJsonConverter.prototype.calculateSimilarity = function (str1, str2) {
    ...
    CsvToJsonConverter.prototype.parseTrueFalseCsv = function () {
        ...
        for (var i = 1; i < lines.length; i++) {
            var fields = this.parseCsvLine(lines[i]);
            if (fields.length >= 4) {
                var id = fields[0], type = fields[1], text = fields[2], correctAnswer = fields[3];
                var question = {
                    id: id.padStart(3, '0'),
                    type: 'true-false',
                    text: text,
                ...
                // 自動添加說明（如果答案是X）
                var explanation = this.getExplanationForQuestion(text, correctAnswer);
                if (explanation) {
